    ...
    public static class BindingScope {
...
#if NOESIS
            AddChildResolver<ItemsControl>(e =>
            {
                ...
                foreach (var item in e.Items)
                {
                    ...
                    {
                        list.Add((DependencyObject)item);
                    }
                }
        ...
        public static Func<ScopeNamingRoute, IEnumerable<FrameworkElement>> FindNamedDescendants = routeHops => {
            ...
            while (queue.Count > 0) {
...
#if NOESIS
                var childCount = (current is Visual)
                    ? VisualTreeHelper.GetChildrenCount((Visual)current) : 0;
#elif NET
                ...
                if (childCount > 0)
                {
                    ...
                    {
                        var childDo = VisualTreeHelper.GetChild((Visual)current, i);
        ...
        public static Func<DependencyObject, ScopeNamingRoute> FindScopeNamingRoute = elementInScope => {
            ...
            while (true) {
                ...
                previous = root;
                root = VisualTreeHelper.GetParent((Visual)previous);
            }
